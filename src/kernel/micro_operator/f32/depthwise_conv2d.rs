...
impl DepthWiseConv2D {
    ...
    pub fn depthwise_conv2d<T: ArrayElem<T>>(
    ) -> BLiteOperator<T> {
        BLiteOperator {
    ...
    pub fn parser<T: ArrayElem<T>>(
        ...
        let depth_multiplier =
            builtin_option.depth_multiplier();
                    ...
                    for in_channel in 0..input_depth {
                        for m in 0..depth_multiplier {
                            let out_channel = m
                                + in_channel
                                    * depth_multiplier;
                            let in_x_origin = (out_x
                                * stride_w)
                                - padding_w;
                            let in_y_origin = (out_y
                                * stride_h)
                                - padding_h;
                            let mut total: T =
                                Default::default();
                            for filter_y in 0..filter_height
                            {
                                    ...
                                    0..filter_width
                                {
                                    ...
                                    {
                                        let input_v_idx = Self::offset(input_height, input_width, input_depth, batch, in_y, in_x, in_channel);
                                        let input_v = input
                                            .data
                                            [input_v_idx
                                                as usize];
                                        let filter_v_idx = Self::offset(filter_height, filter_width, filter_input_depth, 0, filter_y, filter_x, out_channel);
                                        let filter_v = filter
                                                    .data
                                                    [filter_v_idx
                                                        as usize];
